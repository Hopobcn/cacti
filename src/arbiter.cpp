...
Arbiter::Arbiter(double n_req, double flit_size_, double output_len, TechnologyParameter::DeviceType* dt)
...
Arbiter::compute_power() {
    power.readOp.dynamic = (R * arb_req() * Vdd * Vdd / 2 + R * arb_pri() * Vdd * Vdd / 2 + arb_grant() * Vdd * Vdd +
                            arb_int() * 0.5 * Vdd * Vdd);
