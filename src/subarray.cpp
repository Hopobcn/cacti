...
Subarray::Subarray(const DynamicParameter& dp_, bool is_fa_)
    ...
    if (is_fa == false) {
        uint32_t ram_num_cells_wl_stitching = (dp.ram_cell_tech_type == lp_dram) ? dram_num_cells_wl_stitching_
                                                                                 : (dp.ram_cell_tech_type == comm_dram)
                                                                                   ? comm_dram_num_cells_wl_stitching_
                                                                                   : sram_num_cells_wl_stitching_;

        area.h = cell.h * num_rows;
        area.w = cell.w * num_cols + ceil(num_cols / ram_num_cells_wl_stitching) * g_tp.ram_wl_stitching_overhead_ +
                 // stitching overhead
...
//  double r_w_metal = cell.w * g_tp.wire_local.R_per_um;
    ...
    if (dp.is_dram) {
        ...
        if (dp.ram_cell_tech_type == comm_dram) {
            ...
            C_b_row_drain_C = drain_C_(g_tp.dram.cell_a_w, NCH, 1, 0, cell.w, true, true) /
