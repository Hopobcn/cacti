...
bool calculate_time(bool is_tag, int pure_ram, double Nspd, unsigned int Ndwl, unsigned int Ndbl, unsigned int Ndcm,
    ...
    if (flag_results_populate) { //For the final solution, populate the ptr_results data structure  -- TODO: copy only necessary variables
        ...
        ptr_array->delay_senseamp_mux_decoder = MAX(uca->delay_array_to_sa_mux_lev_1_decoder,
        ...
        ptr_array->power_row_predecoder_drivers.readOp.dynamic = uca->bank.mat.r_predec->driver_power.readOp.dynamic *
                                                                 dyn_p.num_act_mats_hor_dir;
        ptr_array->power_row_predecoder_blocks.readOp.dynamic = uca->bank.mat.r_predec->block_power.readOp.dynamic *
                                                                dyn_p.num_act_mats_hor_dir;

        ptr_array->power_row_decoders.readOp.dynamic = uca->bank.mat.power_row_decoders.readOp.dynamic *
                                                       dyn_p.num_act_mats_hor_dir;

        ptr_array->power_bit_mux_predecoder_drivers.readOp.dynamic =
                uca->bank.mat.b_mux_predec->driver_power.readOp.dynamic * dyn_p.num_act_mats_hor_dir;
        ptr_array->power_bit_mux_predecoder_blocks.readOp.dynamic =
                uca->bank.mat.b_mux_predec->block_power.readOp.dynamic * dyn_p.num_act_mats_hor_dir;
        ptr_array->power_bit_mux_decoders.readOp.dynamic = uca->bank.mat.power_bit_mux_decoders.readOp.dynamic *
                                                           dyn_p.num_act_mats_hor_dir;
        ptr_array->power_senseamp_mux_lev_1_predecoder_drivers.readOp.dynamic =
                uca->bank.mat.sa_mux_lev_1_predec->driver_power.readOp.dynamic * dyn_p.num_act_mats_hor_dir;
        ptr_array->power_senseamp_mux_lev_1_predecoder_blocks.readOp.dynamic =
                uca->bank.mat.sa_mux_lev_1_predec->block_power.readOp.dynamic * dyn_p.num_act_mats_hor_dir;
        ptr_array->power_senseamp_mux_lev_1_decoders.readOp.dynamic =
                uca->bank.mat.power_sa_mux_lev_1_decoders.readOp.dynamic * dyn_p.num_act_mats_hor_dir;
        ptr_array->power_senseamp_mux_lev_2_predecoder_drivers.readOp.dynamic =
                uca->bank.mat.sa_mux_lev_2_predec->driver_power.readOp.dynamic * dyn_p.num_act_mats_hor_dir;
        ptr_array->power_senseamp_mux_lev_2_predecoder_blocks.readOp.dynamic =
                uca->bank.mat.sa_mux_lev_2_predec->block_power.readOp.dynamic * dyn_p.num_act_mats_hor_dir;
        ptr_array->power_senseamp_mux_lev_2_decoders.readOp.dynamic =
                uca->bank.mat.power_sa_mux_lev_2_decoders.readOp.dynamic * dyn_p.num_act_mats_hor_dir;
        ...
        ptr_array->power_prechg_eq_drivers.readOp.dynamic = uca->bank.mat.power_bl_precharge_eq_drv.readOp.dynamic *
                                                            dyn_p.num_act_mats_hor_dir;
        ptr_array->power_output_drivers_at_subarray.readOp.dynamic =
                uca->bank.mat.power_subarray_out_drv.readOp.dynamic * dyn_p.num_act_mats_hor_dir;
        ptr_array->power_comparators.readOp.dynamic = uca->bank.mat.power_comparator.readOp.dynamic *
                                                      dyn_p.num_act_mats_hor_dir;
...
void find_optimal_uca(uca_org_t* res, min_values_t* minval, list<uca_org_t>& ulist) {
    ...
    for (list<uca_org_t>::iterator niter = ulist.begin(); niter != ulist.end(); niter++) {
        if (g_ip->ed == 1) {
            ...
            cost = ((niter)->access_time / minval->min_delay) * ((niter)->access_time / minval->min_delay) *
            ...
            } //FIXME remove this after leakage modeling

            if (v) {
                cost = (d * ((niter)->access_time / minval->min_delay) + c * ((niter)->cycle_time / minval->min_cyc) +
