...
void TechnologyParameter::display(uint32_t indent) {
    ...
    cout << indent_str << "dram_cell_I_off_worst_case_len_temp = " << setw(12) << dram_cell_I_off_worst_case_len_temp <<
    " A/um" << endl;
...
DynamicParameter::DynamicParameter(bool is_tag_, int pure_ram_, double Nspd_, unsigned int Ndwl_, unsigned int Ndbl_,
        unsigned int Ndcm, unsigned int Ndsam_lev_1_, unsigned int Ndsam_lev_2_, bool is_main_mem_)
        : is_tag(is_tag_), pure_ram(pure_ram_), tagbits(0), Nspd(Nspd_), Ndwl(Ndwl_), Ndbl(Ndbl_),
          Ndsam_lev_1(Ndsam_lev_1_), Ndsam_lev_2(Ndsam_lev_2_), number_way_select_signals_mat(0), V_b_sense(0),
          use_inp_params(0), is_main_mem(is_main_mem_), cell(), is_valid(false) {
    ...
    if (fully_assoc) { // fully-assocative cache -- ref: CACTi 2.0 report
    ...
    if (is_tag) {
        ...
        if (fully_assoc) {
            ...
            num_r_subarray = (int) (capacity_per_die / (g_ip->nbanks * g_ip->block_sz * g_ip->tag_assoc * Ndbl * Nspd) +
                                    EPSILON);
        ...
        if (fully_assoc) {
            ...
            num_r_subarray = (int) (
                    capacity_per_die / (g_ip->nbanks * g_ip->block_sz * g_ip->data_assoc * Ndbl * Nspd) + EPSILON);
            num_c_subarray = (int) ((8 * g_ip->block_sz * g_ip->data_assoc * Nspd / Ndwl) + EPSILON);
        }
        // burst_length = g_ip->block_sz * 8 / g_ip->out_w;
    }

    if ((!fully_assoc) && (num_r_subarray < MINSUBARRAYROWS)) {
        return;
    }
    if (num_r_subarray == 0) {
        return;
    ...
    if (is_tag) {
        ...
        cell.w = g_tp.sram.b_w +
                 2 * wire_local.pitch * (g_ip->num_rw_ports - 1 + (g_ip->num_rd_ports - g_ip->num_se_rd_ports)) +
        ...
        if (is_dram) {
            ...
            cell.h = g_tp.sram.b_h +
                     2 * wire_local.pitch * (g_ip->num_wr_ports + g_ip->num_rw_ports - 1 + g_ip->num_rd_ports);
            cell.w = g_tp.sram.b_w + 2 * wire_local.pitch *
                                     (g_ip->num_rw_ports - 1 + (g_ip->num_rd_ports - g_ip->num_se_rd_ports) +
    ...
    if (is_dram) {
        ...
        if (ram_cell_tech_type == comm_dram) {
            ...
            dram_refresh_period = 0.9 * g_tp.dram_cell_C * VDD_STORAGE_LOSS_FRACTION_WORST * g_tp.dram_cell_Vdd /
    ...
    if (fully_assoc) {
        ...
        num_mats = num_mats_h_dir * num_mats_v_dir;
        num_do_b_mat = MAX((num_subarrays / num_mats) * num_c_subarray / (deg_bl_muxing * Ndsam_lev_1 * Ndsam_lev_2),
                           1);
    }
    ...
    if (is_dram && is_main_mem) {
        ...
        number_addr_bits_mat = num_addr_b_row_dec + _log2(deg_bl_muxing) + _log2(deg_sa_mux_l1_non_assoc) +
                               _log2(Ndsam_lev_2);
    }
