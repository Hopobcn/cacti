...
double Component::compute_diffusion_width(int num_stacked_in, int num_folded_tr) {
    ...
    double total_diff_w = 2 * spacing_poly_to_poly +  // for both source and drain
                          num_stacked_in * w_poly + (num_stacked_in - 1) * g_tp.spacing_poly_to_poly;

    if (num_folded_tr > 1) {
        total_diff_w += (num_folded_tr - 2) * 2 * spacing_poly_to_poly + (num_folded_tr - 1) * num_stacked_in * w_poly +
...
double Component::compute_gate_area(int gate_type, int num_inputs, double w_pmos, double w_nmos, double h_gate) {
...
int Component::logical_effort(int num_gates_min, double g, double F, double* w_n, double* w_p, double C_load,
        double p_to_n_sz_ratio, bool is_dram_, bool is_wl_tr_, double max_w_nmos) {
