...
Mat::Mat(const DynamicParameter& dyn_p)
        : dp(dyn_p), power_subarray_out_drv(), delay_fa_decoder(0), delay_before_decoder(0), delay_bitline(0),
          delay_wl_reset(0), delay_bl_restore(0), subarray(dp, g_ip->fully_assoc), power_bitline(),
          per_bitline_read_energy(0), deg_bl_muxing(dp.deg_bl_muxing), num_act_mats_hor_dir(dyn_p.num_act_mats_hor_dir),
          delay_writeback(0), cell(subarray.cell), is_dram(dyn_p.is_dram), num_mats(dp.num_mats), power_sa(),
          delay_sa(0), leak_power_sense_amps_closed_page_state(0), leak_power_sense_amps_open_page_state(0),
          delay_subarray_out_drv(0), delay_comparator(0), power_comparator(), num_do_b_mat(dyn_p.num_do_b_mat),
          num_subarrays_per_mat(dp.num_subarrays / dp.num_mats), num_subarrays_per_row(dp.Ndwl / dp.num_mats_h_dir) {
    ...
    if (deg_bl_muxing > 1) {
        C_ld_bit_mux_dec_out = (2 * num_subarrays_per_mat * subarray.num_cols / deg_bl_muxing) *
    ...
    if (dp.Ndsam_lev_2 > 1) {
        C_ld_sa_mux_lev_2_dec_out = (num_subarrays_per_mat * number_sa_subarray / (dp.Ndsam_lev_1 * dp.Ndsam_lev_2)) *
                                    gate_C(g_tp.w_nmos_sa_mux, 0, is_dram) +
                                    num_subarrays_per_row * subarray.num_cols * g_tp.wire_inside_mat.C_per_um *
                                    cell.get_w();
    }
    ...
    row_dec = new Decoder(num_dec_signals, false, subarray.C_wl, R_wire_wl_drv_out, is_fa, is_dram, true, cell);
    if (is_fa && (!dp.is_tag)) {
        row_dec->exist = true;
    }
    bit_mux_dec = new Decoder(deg_bl_muxing, false, C_ld_bit_mux_dec_out, R_wire_bit_mux_dec_out, is_fa, is_dram, false,
                              cell);
    sa_mux_lev_1_dec = new Decoder(dp.deg_senseamp_muxing_non_associativity,
                                   dp.number_way_select_signals_mat ? true : false, C_ld_sa_mux_lev_1_dec_out,
                                   R_wire_sa_mux_dec_out, is_fa, is_dram, false, cell);
    ...
    bl_precharge_eq_drv = new Driver(
            ...
            subarray.num_cols * cell.w * g_tp.wire_outside_mat.R_per_um, is_dram);
    ...
    double h_bit_mux_sense_amp_precharge_sa_mux_write_driver_write_mux = compute_bit_mux_sa_precharge_sa_mux_wr_drv_wr_mux_h();
    ...
    double h_non_cell_area = (num_subarrays_per_mat / num_subarrays_per_row) *
                             (h_bit_mux_sense_amp_precharge_sa_mux_write_driver_write_mux + h_subarray_out_drv +
                              h_comparators);
    ...
    if (!g_ip->ver_htree_wires_over_array) {
        ...
        h_non_cell_area =
                (h_bit_mux_sense_amp_precharge_sa_mux_write_driver_write_mux + h_comparators + h_subarray_out_drv) *
                (num_subarrays_per_mat / num_subarrays_per_row) + h_addr_datain_wires + h_bit_mux_dec_out_wires +
                h_senseamp_mux_dec_out_wires;
    }

    // double area_rectangle_center_mat = h_non_cell_area * w_non_cell_area;
    double area_mat_center_circuitry = (r_predec_blk_drv1->area.get_area() + b_mux_predec_blk_drv1->area.get_area() +
                                        sa_mux_lev_1_predec_blk_drv1->area.get_area() +
                                        sa_mux_lev_2_predec_blk_drv1->area.get_area() + way_sel_drv1->area.get_area() +
                                        r_predec_blk_drv2->area.get_area() + b_mux_predec_blk_drv2->area.get_area() +
                                        sa_mux_lev_1_predec_blk_drv2->area.get_area() +
                                        sa_mux_lev_2_predec_blk_drv2->area.get_area() + r_predec_blk1->area.get_area() +
                                        b_mux_predec_blk1->area.get_area() + sa_mux_lev_1_predec_blk1->area.get_area() +
                                        sa_mux_lev_2_predec_blk1->area.get_area() + r_predec_blk2->area.get_area() +
                                        b_mux_predec_blk2->area.get_area() + sa_mux_lev_1_predec_blk2->area.get_area() +
                                        sa_mux_lev_2_predec_blk2->area.get_area() + bit_mux_dec->area.get_area() +
                                        sa_mux_lev_1_dec->area.get_area() + sa_mux_lev_2_dec->area.get_area()) *
                                       (RWP + ERP + EWP);
...
double Mat::compute_delays(double inrisetime) {
    ...
    if (is_fa && dp.is_tag) {
        delay_fa_tag(dp.tagbits, dp.Ndbl, is_dram);
    } else {
...
double Mat::compute_bit_mux_sa_precharge_sa_mux_wr_drv_wr_mux_h() {
    ...
    if (dp.Ndsam_lev_1 > 1) {
        height += compute_tr_width_after_folding(g_tp.w_nmos_sa_mux,
                                                 cell.w * deg_bl_muxing / (RWP + ERP));  // sense_amp_mux_height
        //height_senseamp_mux_decode_output_wires =  Ndsam * wire_inside_mat_pitch * (RWP + ERP);
    }

    if (dp.Ndsam_lev_2 > 1) {
        height += compute_tr_width_after_folding(g_tp.w_nmos_sa_mux, cell.w * deg_bl_muxing * dp.Ndsam_lev_1 /
                                                                     (RWP + ERP));  // sense_amp_mux_height
        //height_senseamp_mux_decode_output_wires =  Ndsam * wire_inside_mat_pitch * (RWP + ERP);

        // add height of inverter-buffers between the two levels (pass-transistors) of sense-amp mux
        height += 2 * compute_tr_width_after_folding(pmos_to_nmos_sz_ratio(is_dram) * g_tp.min_w_nmos_,
...
void Mat::delay_fa_tag(int tagbits, int Ntbl, bool is_dram) {
    double Tagdrive1, Tagdrive2, Tag1, Tag2, Tag3, outrisetime;
    double Ceq, Rwire, tf, nextinputtime, c_intrinsic, rd, Cwire, c_gate_load;

    double Wdecdrivep, Wdecdriven, Wfadriven, Wfadrivep, Wfadrive2n, Wfadrive2p, Wfadecdrive1n, Wfadecdrive1p, Wfadecdrive2n, Wfadecdrive2p, Wfadecdriven, Wfadecdrivep, Wfaprechn, Wfaprechp, Wdummyn, Wdummyinvn, Wdummyinvp, Wfainvn, Wfainvp, Waddrnandn, Waddrnandp, Wfanandn, Wfanandp, Wfanorn, Wfanorp, Wdecnandn, Wdecnandp;
...
double Mat::compute_comparators_height(int tagbits, int number_ways_in_mat, double subarray_mem_cell_area_width) {
...
double Mat::compute_bitline_delay(double inrisetime) {
    ...
    double C_drain_sense_amp_mux = drain_C_(g_tp.w_nmos_sa_mux, NCH, 1, 0, cell.w * deg_bl_muxing / (RWP + ERP),
                                            is_dram);
    ...
    if (is_dram) {
        ...
        dynWriteEnergy += (C_bl + 2 * C_drain_sense_amp_iso + C_sense_amp_latch) * (g_tp.dram_cell_Vdd / 2) *
                          g_tp.dram_cell_Vdd * subarray.num_cols * num_subarrays_per_mat * num_act_mats_hor_dir;
        ...
        if (deg_bl_muxing > 1) {
            tau = (R_cell_pull_down + R_cell_acc) *
                  ...
                  R_bl * (C_bl / 2 + 2 * C_drain_bit_mux + 2 * C_drain_sense_amp_iso + C_sense_amp_latch +
                          C_drain_sense_amp_mux) + R_bit_mux *
                                                   (C_drain_bit_mux + 2 * C_drain_sense_amp_iso + C_sense_amp_latch +
                                                    C_drain_sense_amp_mux) +
                  R_sense_amp_iso * (C_drain_sense_amp_iso + C_sense_amp_latch + C_drain_sense_amp_mux);
            dynRdEnergy += (C_bl + 2 * C_drain_bit_mux) * 2 * dp.V_b_sense * g_tp.sram_cell.Vdd * subarray.num_cols *
                           num_subarrays_per_mat;
            dynRdEnergy += (2 * C_drain_sense_amp_iso + C_sense_amp_latch + C_drain_sense_amp_mux) * 2 * dp.V_b_sense *
                           g_tp.sram_cell.Vdd * (subarray.num_cols * num_subarrays_per_mat / deg_bl_muxing);
            dynWriteEnergy += ((subarray.num_cols * num_subarrays_per_mat / deg_bl_muxing) / deg_senseamp_muxing) *
                              num_act_mats_hor_dir * (C_bl + 2 * C_drain_bit_mux) * g_tp.sram_cell.Vdd *
                              g_tp.sram_cell.Vdd;
        } else {
            tau = (R_cell_pull_down + R_cell_acc) *
                  (C_bl + C_drain_sense_amp_iso + C_sense_amp_latch + C_drain_sense_amp_mux) + R_bl * C_bl / 2 +
                  R_sense_amp_iso * (C_drain_sense_amp_iso + C_sense_amp_latch + C_drain_sense_amp_mux);
            dynRdEnergy += (C_bl + 2 * C_drain_sense_amp_iso + C_sense_amp_latch + C_drain_sense_amp_mux) * 2 *
                           dp.V_b_sense * g_tp.sram_cell.Vdd * subarray.num_cols * num_subarrays_per_mat;
            dynWriteEnergy += (((subarray.num_cols * num_subarrays_per_mat / deg_bl_muxing) / deg_senseamp_muxing) *
                               num_act_mats_hor_dir * C_bl) * g_tp.sram_cell.Vdd * g_tp.sram_cell.Vdd;

        }
        tstep = tau * log(V_b_pre / (V_b_pre - dp.V_b_sense));
        power_bitline.readOp.leakage = leak_power_cc_inverters_sram_cell + leak_power_acc_tr_RW_or_WR_port_sram_cell +
...
double Mat::compute_sa_delay(double inrisetime) {
    ...
    leak_power_sense_amps_closed_page_state = lkgIdlePh * g_tp.peri_global.Vdd * num_sa_subarray *
                                              num_subarrays_per_mat;
...
double Mat::compute_comparator_delay(double inrisetime) {
    ...
    if ((tstep) <= (0.5 * (g_tp.peri_global.Vdd - g_tp.peri_global.Vth) / m)) {
        ...
        double c = -2 * (tstep) * (g_tp.peri_global.Vdd - g_tp.peri_global.Vth) +
        ...
        Tcomparatorni = (tstep) + (g_tp.peri_global.Vdd + g_tp.peri_global.Vth) / (2 * m) -
...
void Mat::compute_power_energy() {
    ...
    power.readOp.dynamic += r_predec->power.readOp.dynamic + b_mux_predec->power.readOp.dynamic +
                            sa_mux_lev_1_predec->power.readOp.dynamic + sa_mux_lev_2_predec->power.readOp.dynamic;
    ...
    if (is_fa == false || dp.is_tag == false) {
        ...
        power.readOp.dynamic += power_bl_precharge_eq_drv.readOp.dynamic + power_sa.readOp.dynamic +
                                power_bitline.readOp.dynamic + power_subarray_out_drv.readOp.dynamic;
    }

    power.readOp.dynamic += power_row_decoders.readOp.dynamic + bit_mux_dec->power.readOp.dynamic +
                            sa_mux_lev_1_dec->power.readOp.dynamic + sa_mux_lev_2_dec->power.readOp.dynamic +
                            power_comparator.readOp.dynamic;


    // calculate leakage power
    if (is_fa == false || dp.is_tag ==
                          false) { // if fully associative cache and tag array, don't add the following components of leakage power
        ...
        power.readOp.leakage += power_bitline.readOp.leakage + power_bl_precharge_eq_drv.readOp.leakage +
                                power_sa.readOp.leakage + power_subarray_out_drv.readOp.leakage;
    }
    ...
    power.readOp.leakage += r_predec->power.readOp.leakage + b_mux_predec->power.readOp.leakage +
                            sa_mux_lev_1_predec->power.readOp.leakage + sa_mux_lev_2_predec->power.readOp.leakage +
                            power_row_decoders.readOp.leakage + power_bit_mux_decoders.readOp.leakage +
                            power_sa_mux_lev_1_decoders.readOp.leakage + power_sa_mux_lev_2_decoders.readOp.leakage;
}
