...
Htree2::Htree2(enum Wire_type wire_model, double mat_w, double mat_h, int a_bits, int d_inbits, int d_outbits, int bl,
        int wl, enum Htree_type htree_type, bool uca_tree_, TechnologyParameter::DeviceType* dt)
        : in_rise_time(0), out_rise_time(0), tree_type(htree_type), mat_width(mat_w), mat_height(mat_h),
          add_bits(a_bits), data_in_bits(d_inbits), data_out_bits(d_outbits), ndbl(bl), ndwl(wl), uca_tree(uca_tree_),
...
void Htree2::input_nand(double s1, double s2, double l_eff) {
    ...
    power.readOp.dynamic += 0.5 * (2 * drain_C_(pton_size * nsize * min_w_pmos, PCH, 1, 1, g_tp.cell_h_def) +
                                   drain_C_(nsize * min_w_nmos, NCH, 1, 1, g_tp.cell_h_def) +
                                   2 * gate_C(s2 * (min_w_nmos + min_w_pmos), 0)) * deviceType->Vdd * deviceType->Vdd;

    power.readOp.leakage += (wire_bw * cmos_Ileak(min_w_nmos * (nsize * 2), min_w_pmos * nsize * 2, false) *
                             deviceType->Vdd);
}
...
void Htree2::output_buffer(double s1, double s2, double l_eff) {
    ...
    double cap_nand_out = drain_C_(size * min_w_nmos, NCH, 1, 1, g_tp.cell_h_def) +
                          drain_C_(size * min_w_pmos, PCH, 1, 1, g_tp.cell_h_def) * 2 + gate_C(tr_size * min_w_pmos, 0);
    ...
    //nand
    power.readOp.dynamic += 0.5 * (2 * drain_C_(size * min_w_pmos, PCH, 1, 1, g_tp.cell_h_def) +
                                   drain_C_(size * min_w_nmos, NCH, 1, 1, g_tp.cell_h_def) +
                                   gate_C(tr_size * (min_w_pmos), 0)) * deviceType->Vdd * deviceType->Vdd;

    //not
    power.readOp.dynamic += 0.5 * (drain_C_(size * min_w_pmos, PCH, 1, 1, g_tp.cell_h_def) +
                                   drain_C_(size * min_w_nmos, NCH, 1, 1, g_tp.cell_h_def) +
                                   gate_C(size * (min_w_nmos + min_w_pmos), 0)) * deviceType->Vdd * deviceType->Vdd;

    //nor
    power.readOp.dynamic += 0.5 * (drain_C_(size * min_w_pmos, PCH, 1, 1, g_tp.cell_h_def) +
                                   2 * drain_C_(size * min_w_nmos, NCH, 1, 1, g_tp.cell_h_def) +
                                   gate_C(tr_size * (min_w_nmos + min_w_pmos), 0)) * deviceType->Vdd * deviceType->Vdd;

    //output transistor
    power.readOp.dynamic += 0.5 * ((drain_C_(tr_size * min_w_pmos, PCH, 1, 1, g_tp.cell_h_def) +
                                    drain_C_(tr_size * min_w_nmos, NCH, 1, 1, g_tp.cell_h_def)) * 2 +
                                   gate_C(s1 * (min_w_nmos + min_w_pmos), 0)) * deviceType->Vdd * deviceType->Vdd;

    if (uca_tree) {
        power.readOp.leakage += (deviceType->Vdd * cmos_Ileak(
                min_w_nmos * (tr_size * 2/*inverter + output tr*/ + size * 6 /*nand + nor*/),
...
Htree2::in_htree() {
    ...
    if (uca_tree) {
        ht_temp = (mat_height * ndbl / 2 +/* since uca_tree models interbank tree, mat_height => bank height */
                   ((add_bits + data_in_bits + data_out_bits) * g_tp.wire_outside_mat.pitch * 2 * (1 - pow(0.5, h)))) /
                  2;
        len_temp = (mat_width * ndwl / 2 +
                    ((add_bits + data_in_bits + data_out_bits) * g_tp.wire_outside_mat.pitch * 2 * (1 - pow(0.5, v)))) /
                   2;
    } else {
        if (ndwl == ndbl) {
            ht_temp = ((mat_height * ndbl / 2) + (add_bits * (ndbl / 2 - 1) * g_tp.wire_outside_mat.pitch) +
                       ((data_in_bits + data_out_bits) * g_tp.wire_outside_mat.pitch * h)) / 2;
            len_temp = (mat_width * ndwl / 2 + (add_bits * (ndwl / 2 - 1) * g_tp.wire_outside_mat.pitch) +
            ...
            ht_temp = ((mat_height * ndbl / 2) +
                       ...
                       ((data_in_bits + data_out_bits) * g_tp.wire_outside_mat.pitch * h)) / 2;
            len_temp =
                    (mat_width * ndwl / 2 + (add_bits * ((ndwl / 2 - 1) + excess_part) * g_tp.wire_outside_mat.pitch) +
                     (data_in_bits + data_out_bits) * g_tp.wire_outside_mat.pitch * (h + 2 * (1 - pow(0.5, v - h)))) /
                    2;
        }
    }
