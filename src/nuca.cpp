...
Nuca::sim_nuca() {
    ...
    if (g_ip->nuca_bank_count != 0) { // simulate just one bank
        if (g_ip->nuca_bank_count != 2 && g_ip->nuca_bank_count != 4 && g_ip->nuca_bank_count != 8 &&
            g_ip->nuca_bank_count != 16 && g_ip->nuca_bank_count != 32 && g_ip->nuca_bank_count != 64) {
...
//    output_UCA(&ures);
        ...
        for (wr = wt_min; wr <= wt_max; wr++) {
            ...
            for (ro = 0; ro < ROUTER_TYPES; ro++) {
                ...
                ver_hop_lat = calc_cycles(wire_vertical[wr]->delay, 1 / (nuca_list.back()->nuca_pda.cycle_time * .001));
                ...
                for (c = 1; c <= (unsigned int) bank_count; c++) {
                    ...
                    curr_acclat = 2 * avg_lat + 2 * (router_s[ro]->delay * avg_hop) +
                                  calc_cycles(ures.access_time, 1 / (nuca_list.back()->nuca_pda.cycle_time * .001));

                    /* avg access lat of nuca */
                    avg_dyn_power = avg_hop * (router_s[ro]->power.readOp.dynamic) +
                                    avg_hhop * (wire_horizontal[wr]->power.readOp.dynamic) * (g_ip->block_sz * 8 + 64) +
                                    avg_vhop * (wire_vertical[wr]->power.readOp.dynamic) * (g_ip->block_sz * 8 + 64) +
                                    ures.power.readOp.dynamic;

                    avg_leakage_power = bank_count * router_s[ro]->power.readOp.leakage + avg_hhop *
                ...
                nuca_list.back()->wire_pda.power.readOp.dynamic = opt_avg_hop * flit_width *
                ...
                if (num_cyc > 16) {
                    num_cyc = 16;
                } // we have data only up to 16 cycles

                if (it < 7) {
                    nuca_list.back()->nuca_pda.delay = opt_acclat + cont_stats[l2_c][core_in][ro][it][num_cyc / 2 - 1];
                    nuca_list.back()->contention = cont_stats[l2_c][core_in][ro][it][num_cyc / 2 - 1];
                } else {
                    nuca_list.back()->nuca_pda.delay = opt_acclat + cont_stats[l2_c][core_in][ro][7][num_cyc / 2 - 1];
                    nuca_list.back()->contention = cont_stats[l2_c][core_in][ro][7][num_cyc / 2 - 1];
                }
...
Nuca::print_nuca(nuca_org_t* fr) {
    ...
    printf("Cache dimension (mm x mm) - %g x %g\n", fr->nuca_pda.area.h * 1e-3, fr->nuca_pda.area.w * 1e-3);
...
nuca_org_t* Nuca::find_optimal_nuca(list<nuca_org_t*>* n, min_values_t* minval) {
    ...
    for (niter = n->begin(); niter != n->end(); niter++) {
        ...
        printf("NUCA___stats %d \tbankcount: lat = %g \tdynP = %g \twt = %d\t "
               ...
               (*niter)->nuca_pda.power.readOp.dynamic, (*niter)->h_wire->wt, (*niter)->bank_pda.power.readOp.dynamic,
        ...
        if (g_ip->ed == 1) {
            ...
            cost = ((*niter)->nuca_pda.delay / minval->min_delay) * ((*niter)->nuca_pda.delay / minval->min_delay) *
                   ((*niter)->nuca_pda.power.readOp.dynamic / minval->min_dyn);
            if (min_cost > cost) {
                min_cost = cost;
                res = ((*niter));
...
Nuca::check_nuca_org(nuca_org_t* n, min_values_t* minval) {
    ...
    if (((n->nuca_pda.power.readOp.dynamic - minval->min_dyn) / minval->min_dyn) * 100 > g_ip->dynamic_power_dev_nuca) {
        return 0;
    }
    ...
    if (((n->nuca_pda.cycle_time - minval->min_cyc) / minval->min_cyc) * 100 > g_ip->cycle_time_dev_nuca) {
        return 0;
    }
    if (((n->nuca_pda.area.get_area() - minval->min_area) / minval->min_area) * 100 > g_ip->area_dev_nuca) {
        return 0;
    }
    return 1;
}
