...
Decoder::Decoder(int _num_dec_signals, bool flag_way_select, double _C_ld_dec_out, double _R_wire_dec_out,
        bool fully_assoc_, bool is_dram_, bool is_wl_tr_, const Area& cell_)
        : exist(false), C_ld_dec_out(_C_ld_dec_out), R_wire_dec_out(_R_wire_dec_out), num_gates(0), num_gates_min(2),
          delay(0), power(), fully_assoc(fully_assoc_), is_dram(is_dram_), is_wl_tr(is_wl_tr_), cell(cell_) {
...
void Decoder::compute_widths() {
    ...
    if (exist) {
        ...
        F *= C_ld_dec_out /
             (gate_C(w_dec_n[0], 0, is_dram, false, is_wl_tr) + gate_C(w_dec_p[0], 0, is_dram, false, is_wl_tr));
        num_gates = logical_effort(num_gates_min, num_in_signals == 2 ? gnand2 : gnand3, F, w_dec_n, w_dec_p,
                                   C_ld_dec_out, p_to_n_sz_ratio, is_dram, is_wl_tr, g_tp.max_w_nmos_dec);
    }
}
...
PredecBlk::PredecBlk(int num_dec_signals, Decoder* dec_, double C_wire_predec_blk_out, double R_wire_predec_blk_out_,
        int num_dec_per_predec, bool is_dram, bool is_blk1)
        : dec(dec_), exist(false), number_input_addr_bits(0), C_ld_predec_blk_out(0), R_wire_predec_blk_out(0),
          branch_effort_nand2_gate_output(1), branch_effort_nand3_gate_output(1), flag_two_unique_paths(false),
          flag_L2_gate(0), number_inputs_L1_gate(0), number_gates_L1_nand2_path(0), number_gates_L1_nand3_path(0),
          number_gates_L2(0), min_number_gates_L1(2), min_number_gates_L2(2), delay_nand2_path(0), delay_nand3_path(0),
          power_nand2_path(), power_nand3_path(), power_L2(), is_dram_(is_dram) {
...
void PredecBlk::compute_widths() {
    ...
    if (flag_L2_gate) {
        ...
        number_gates_L2 = logical_effort(min_number_gates_L2, flag_L2_gate == 2 ? gnand2 : gnand3, F, w_L2_n, w_L2_p,
                                         C_ld_predec_blk_out, p_to_n_sz_ratio, is_dram_, false, g_tp.max_w_nmos_);

        // Now find the number of gates and widths in first level of predecoder
        if ((flag_two_unique_paths) || (number_inputs_L1_gate ==
                                        2)) { // Whenever flag_two_unique_paths is true, it means first level of decoder employs
            ...
            F = gnand2 * c_load_nand2_path /
                (gate_C(w_L1_nand2_n[0], 0, is_dram_) + gate_C(w_L1_nand2_p[0], 0, is_dram_));
            number_gates_L1_nand2_path = logical_effort(min_number_gates_L1, gnand2, F, w_L1_nand2_n, w_L1_nand2_p,
                                                        c_load_nand2_path, p_to_n_sz_ratio, is_dram_, false,
                                                        g_tp.max_w_nmos_);
        }
        ...
        if ((flag_two_unique_paths) || (number_inputs_L1_gate ==
            ...
            F = gnand3 * c_load_nand3_path /
                (gate_C(w_L1_nand3_n[0], 0, is_dram_) + gate_C(w_L1_nand3_p[0], 0, is_dram_));
            number_gates_L1_nand3_path = logical_effort(min_number_gates_L1, gnand3, F, w_L1_nand3_n, w_L1_nand3_p,
                                                        c_load_nand3_path, p_to_n_sz_ratio, is_dram_, false,
                                                        g_tp.max_w_nmos_);
        }
        ...
        if (number_inputs_L1_gate == 2) {
            w_L1_nand2_n[0] = 2 * g_tp.min_w_nmos_;
            w_L1_nand2_p[0] = p_to_n_sz_ratio * g_tp.min_w_nmos_;
            F = C_ld_predec_blk_out / (gate_C(w_L1_nand2_n[0], 0, is_dram_) + gate_C(w_L1_nand2_p[0], 0, is_dram_));
            number_gates_L1_nand2_path = logical_effort(min_number_gates_L1, gnand2, F, w_L1_nand2_n, w_L1_nand2_p,
                                                        C_ld_predec_blk_out, p_to_n_sz_ratio, is_dram_, false,
                                                        g_tp.max_w_nmos_);
        } else if (number_inputs_L1_gate == 3) {
            w_L1_nand3_n[0] = 3 * g_tp.min_w_nmos_;
            w_L1_nand3_p[0] = p_to_n_sz_ratio * g_tp.min_w_nmos_;
            F = C_ld_predec_blk_out / (gate_C(w_L1_nand3_n[0], 0, is_dram_) + gate_C(w_L1_nand3_p[0], 0, is_dram_));
            number_gates_L1_nand3_path = logical_effort(min_number_gates_L1, gnand3, F, w_L1_nand3_n, w_L1_nand3_p,
                                                        C_ld_predec_blk_out, p_to_n_sz_ratio, is_dram_, false,
                                                        g_tp.max_w_nmos_);
        }
...
pair<double, double> PredecBlk::compute_delays(pair<double, double> inrisetime)  // <nand2, nand3>
{
    ...
    if (exist) {
        ...
        if ((flag_two_unique_paths) ||
            (number_inputs_L1_gate == 3)) { //Check if the number of gates in the first level is more than 1. 
...
PredecBlkDrv::PredecBlkDrv(int way_select_, PredecBlk* blk_, bool is_dram)
        : flag_driver_exists(0), number_gates_nand2_path(0), number_gates_nand3_path(0), min_number_gates(2),
          num_buffers_driving_1_nand2_load(0), num_buffers_driving_2_nand2_load(0), num_buffers_driving_4_nand2_load(0),
          num_buffers_driving_2_nand3_load(0), num_buffers_driving_8_nand3_load(0), num_buffers_nand3_path(0),
          c_load_nand2_path_out(0), c_load_nand3_path_out(0), r_load_nand2_path_out(0), r_load_nand3_path_out(0),
          delay_nand2_path(0), delay_nand3_path(0), power_nand2_path(), power_nand3_path(), blk(blk_), dec(blk->dec),
          is_dram_(is_dram), way_select(way_select_) {
    for (int i = 0; i < MAX_NUMBER_GATES_STAGE; i++) {
...
void PredecBlkDrv::compute_widths() {
    ...
    if (flag_driver_exists) {
        ...
        if (way_select == 0) {
            ...
            } else if (blk->number_input_addr_bits == 2) { //4 NAND2 gates
                num_buffers_driving_4_nand2_load = 2;
                c_load_nand2_path_out = 4 * C_nand2_gate_blk;
            } else if (blk->number_input_addr_bits == 3) { //8 NAND3 gates
                num_buffers_driving_8_nand3_load = 3;
                c_load_nand3_path_out = 8 * C_nand3_gate_blk;
            } else if (blk->number_input_addr_bits == 4) { //4 + 4 NAND2 gates
                num_buffers_driving_4_nand2_load = 4;
                c_load_nand2_path_out = 4 * C_nand2_gate_blk;
            } else if (blk->number_input_addr_bits == 5) { //4 NAND2 gates, 8 NAND3 gates
                num_buffers_driving_4_nand2_load = 2;
                num_buffers_driving_8_nand3_load = 3;
                c_load_nand2_path_out = 4 * C_nand2_gate_blk;
                c_load_nand3_path_out = 8 * C_nand3_gate_blk;
            } else if (blk->number_input_addr_bits == 6) { //8 + 8 NAND3 gates
                num_buffers_driving_8_nand3_load = 6;
                c_load_nand3_path_out = 8 * C_nand3_gate_blk;
            } else if (blk->number_input_addr_bits == 7) { //4 + 4 NAND2 gates, 8 NAND3 gates
                num_buffers_driving_4_nand2_load = 4;
                num_buffers_driving_8_nand3_load = 3;
                c_load_nand2_path_out = 4 * C_nand2_gate_blk;
                c_load_nand3_path_out = 8 * C_nand3_gate_blk;
            } else if (blk->number_input_addr_bits == 8) { //4 NAND2 gates, 8 + 8 NAND3 gates
                num_buffers_driving_4_nand2_load = 2;
                num_buffers_driving_8_nand3_load = 6;
                c_load_nand2_path_out = 4 * C_nand2_gate_blk;
                c_load_nand3_path_out = 8 * C_nand3_gate_blk;
            } else if (blk->number_input_addr_bits == 9) { //8 + 8 + 8 NAND3 gates
                num_buffers_driving_8_nand3_load = 9;
                c_load_nand3_path_out = 8 * C_nand3_gate_blk;
            }
        }

        if ((blk->flag_two_unique_paths) || (blk->number_inputs_L1_gate == 2) || (number_input_addr_bits == 0) ||
            ((way_select) && (dec->num_in_signals == 2))) { //this means that way_select is driving NAND2 in decoder. 
            ...
            number_gates_nand2_path = logical_effort(min_number_gates, 1, F, width_nand2_path_n, width_nand2_path_p,
                                                     c_load_nand2_path_out, p_to_n_sz_ratio, is_dram_, false,
                                                     g_tp.max_w_nmos_);
        }

        if ((blk->flag_two_unique_paths) || (blk->number_inputs_L1_gate == 3) ||
            ((way_select) && (dec->num_in_signals == 3))) { //this means that way_select is driving NAND3 in decoder. 
            ...
            number_gates_nand3_path = logical_effort(min_number_gates, 1, F, width_nand3_path_n, width_nand3_path_p,
                                                     c_load_nand3_path_out, p_to_n_sz_ratio, is_dram_, false,
                                                     g_tp.max_w_nmos_);
        }
    }
}
...
void PredecBlkDrv::compute_area() {
    ...
    if (flag_driver_exists) { // first check whether a predecoder block driver is needed
        for (int i = 0; i < number_gates_nand2_path; ++i) {
            ...
            leak_nand2_path += cmos_Ileak(width_nand2_path_n[i], width_nand2_path_p[i], is_dram_) *
                               INV_LEAK_STACK_FACTOR;
        }
        area_nand2_path *= (num_buffers_driving_1_nand2_load + num_buffers_driving_2_nand2_load +
        ...
        for (int i = 0; i < number_gates_nand3_path; ++i) {
            ...
            leak_nand3_path += cmos_Ileak(width_nand3_path_n[i], width_nand3_path_p[i], is_dram_) *
                               INV_LEAK_STACK_FACTOR;
        }
...
pair<double, double> PredecBlkDrv::compute_delays(double inrisetime_nand2_path, double inrisetime_nand3_path) {
    pair<double, double> ret_val;
    ret_val.first = 0;  // outrisetime_nand2_path 
    ret_val.second = 0;  // outrisetime_nand3_path
    int i;
...
Predec::Predec(PredecBlkDrv* drv1_, PredecBlkDrv* drv2_)
    ...
    driver_power.readOp.leakage = drv1->power_nand2_path.readOp.leakage + drv1->power_nand3_path.readOp.leakage +
                                  drv2->power_nand2_path.readOp.leakage + drv2->power_nand3_path.readOp.leakage;
    block_power.readOp.leakage = blk1->power_nand2_path.readOp.leakage + blk1->power_nand3_path.readOp.leakage +
                                 blk1->power_L2.readOp.leakage + blk2->power_nand2_path.readOp.leakage +
...
double Predec::compute_delays(double inrisetime) {
    ...
    driver_power.readOp.dynamic = drv1->num_addr_bits_nand2_path() * drv1->power_nand2_path.readOp.dynamic +
    ...
    block_power.readOp.dynamic = blk1->power_nand2_path.readOp.dynamic + blk1->power_nand3_path.readOp.dynamic +
                                 blk1->power_L2.readOp.dynamic + blk2->power_nand2_path.readOp.dynamic +
                                 blk2->power_nand3_path.readOp.dynamic + blk2->power_L2.readOp.dynamic;
...
pair<double, double> Predec::get_max_delay_before_decoder(pair<double, double> input_pair1,
...
Driver::Driver(double c_gate_load_, double c_wire_load_, double r_wire_load_, bool is_dram)
        : number_gates(0), min_number_gates(2), c_gate_load(c_gate_load_), c_wire_load(c_wire_load_),
          r_wire_load(r_wire_load_), delay(0), power(), is_dram_(is_dram) {
    for (int i = 0; i < MAX_NUMBER_GATES_STAGE; i++) {
        width_n[i] = 0;
        width_p[i] = 0;
    }
...
void Driver::compute_widths() {
    ...
    number_gates = logical_effort(min_number_gates, 1, F, width_n, width_p, c_load, p_to_n_sz_ratio, is_dram_, false,
                                  g_tp.max_w_nmos_);
}
...
double Driver::compute_delay(double inrisetime) {
    ...
    for (i = 0; i < number_gates - 1; ++i) {
        ...
        power.readOp.leakage += cmos_Ileak(width_n[i], width_p[i], is_dram_) * 0.5 * g_tp.peri_global.Vdd;
    }
