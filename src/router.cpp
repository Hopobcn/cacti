...
Router::Router(double flit_size_, double vc_buf, /* vc size = vc_buffer_size * flit_size */
        double vc_c, TechnologyParameter::DeviceType* dt)
...
double
Router::diff_cap(double w, int type /*0 for n-mos and 1 for p-mos*/, double s /*number of stacking transistors*/) {
...
Router::crossbar_inpline() {
    return (Cw3(O * flit_size * wt) + O * transmission_buf_inpcap() + gate_cap(NTid) + gate_cap(PTid) +
            diff_cap(NTid, 0, 1) + diff_cap(PTid, 1, 1));
}

double
Router::crossbar_outline() {
    return (Cw3(I * flit_size * ht) + I * transmission_buf_outcap() + gate_cap(NTod) + gate_cap(PTod) +
            diff_cap(NTod, 0, 1) + diff_cap(PTod, 1, 1));
}

double
Router::crossbar_ctrline() {
    return (Cw3(0.5 * O * flit_size * wt) + flit_size * transmission_buf_ctrcap() + diff_cap(NTi, 0, 1) +
...
void Router::buffer_stats() {
    ...
    dyn_p.ram_cell_tech_type = dyn_p.num_r_subarray = (int) vc_buffer_size;
    ...
    dyn_p.cell.h = g_tp.sram.b_h +
                   2 * g_tp.wire_outside_mat.pitch * (dyn_p.num_wr_ports + dyn_p.num_rw_ports - 1 + dyn_p.num_rd_ports);
    dyn_p.cell.w = g_tp.sram.b_w + 2 * g_tp.wire_outside_mat.pitch *
                                   (dyn_p.num_rw_ports - 1 + (dyn_p.num_rd_ports - dyn_p.num_se_rd_ports) +
...
Router::cb_stats() {
    if (1) {
        ...
        crossbar.power.readOp.leakage = flit_size * I * O * cmos_Ileak(NTtr * g_tp.min_w_nmos_, PTtr * min_w_pmos);
    }
}
...
Router::get_router_power() {
    ...
    vcarb.compute_power();
    cbarb.compute_power();
    arbiter.power.readOp.dynamic = vcarb.power.readOp.dynamic * I + cbarb.power.readOp.dynamic * O;
    arbiter.power.readOp.leakage = vcarb.power.readOp.leakage * I + cbarb.power.readOp.leakage * O;

//  arb_stats();
    power.readOp.dynamic =
            (I * (buffer.power.readOp.dynamic * buffer.power.writeOp.dynamic) + crossbar.power.readOp.dynamic +
